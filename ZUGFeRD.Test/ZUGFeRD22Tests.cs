...
namespace s2industries.ZUGFeRD.Test
{
    ...
    public class ZUGFeRD22Tests : TestBase
    {
        ...
        public void TestLoadingSepaPreNotification()
        {
            ...
            var firstSettlement = invoiceDescriptor.SpecifiedTradeSettlementPaymentMeans.FirstOrDefault();
            Assert.IsNotNull(firstSettlement);
            Assert.AreEqual("DE98ZZZ09999999999", firstSettlement.SEPACreditorIdentifier);
            Assert.AreEqual("REF A-123", firstSettlement.SEPAMandateReference);
            Assert.AreEqual(PaymentMeansTypeCodes.SEPADirectDebit, firstSettlement.TypeCode);
            Assert.IsNotNull(firstSettlement.DebitorBankAccount);
            Assert.AreEqual("DE21860000000086001055", firstSettlement.DebitorBankAccount.IBAN);
        ...
        public void TestStoringSepaPreNotification()
        {
            ...
            using (var stream = new MemoryStream())
            {
                ...
                var firstSettlement = d2.SpecifiedTradeSettlementPaymentMeans.FirstOrDefault();
                Assert.IsNotNull(firstSettlement);
                Assert.AreEqual("DE98ZZZ09999999999", firstSettlement.SEPACreditorIdentifier);
                Assert.AreEqual("REF A-123", firstSettlement.SEPAMandateReference);
                Assert.IsNotNull(firstSettlement.DebitorBankAccount);
        ...
        public void TestAdditionalReferencedDocument()
        {
            ...
            ms.Seek(0, SeekOrigin.Begin);
            InvoiceDescriptor loadedInvoice = InvoiceDescriptor.Load(ms);
            Assert.HasCount(2, loadedInvoice.AdditionalReferencedDocuments);
        ...
        public void TestParty_WithTaxRegistration()
        {
            ...
            Assert.IsNotNull(loadedInvoice.ShipTo);
            Assert.HasCount(1, loadedInvoice.GetShipToTaxRegistration());
        ...
        public void TestWriteAndReadExtended()
        {
            ...
            var firstSettlement = loadedInvoice.SpecifiedTradeSettlementPaymentMeans.FirstOrDefault();
            Assert.IsNotNull(firstSettlement);
            Assert.AreEqual(PaymentMeansTypeCodes.SEPACreditTransfer, firstSettlement.TypeCode);
            Assert.AreEqual("Zahlung per SEPA Überweisung.", firstSettlement.Information);
            Assert.AreEqual("", firstSettlement.SEPACreditorIdentifier);
            Assert.AreEqual("SepaMandat", firstSettlement.SEPAMandateReference);
            Assert.AreEqual("123", firstSettlement.FinancialCard.Id);
            Assert.AreEqual("Mustermann", firstSettlement.FinancialCard.CardholderName);

            Assert.IsNotNull(firstSettlement.CreditorBankAccount);
            Assert.AreEqual("Kunden AG", firstSettlement.CreditorBankAccount.Name);
            Assert.AreEqual("DE02120300000000202051", firstSettlement.CreditorBankAccount.IBAN);
            Assert.AreEqual("BYLADEM1001", firstSettlement.CreditorBankAccount.BIC);

            var debitorBankAccount = loadedInvoice.SpecifiedTradeSettlementPaymentMeans
                .Select(q => q.DebitorBankAccount)
                .FirstOrDefault(a => a.IBAN == "DE02120300000000202052");
            ...
            Assert.IsFalse(string.IsNullOrEmpty(loadedLineItem.AssociatedDocument.LineID));
            ...
            Assert.HasCount(1, loadedLineItem.GetAdditionalReferencedDocuments());
        ...
        public void TestDesignatedProductClassificationWithFullClassification()
        {
            ...
            Assert.Contains("<ram:DesignatedProductClassification>", content);
            Assert.Contains("<ram:ClassCode listID=\"HS\" listVersionID=\"List Version ID Value\">Class Code</ram:ClassCode>", content);
            Assert.Contains("<ram:ClassName>Class Name</ram:ClassName>", content);
        ...
        **/
        ...
        public void TestSingleXRechnungStructuredManually()
        {
            ...
            foreach (var line in lines)
            {
                ...
                if (!insideDescription && trimmedLine.StartsWith("<ram:Description>", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    Assert.IsGreaterThanOrEqualTo(0, noteIndentation, "Indentation for <ram:Description> should be non-negative.");
        ...
        public void TestRequiredDirectDebitFieldsShouldExist()
        {
            ...
            using (var stream = new MemoryStream())
            {
                ...
                Assert.Contains($"<ram:CreditorReferenceID>DE98ZZZ09999999999</ram:CreditorReferenceID>", content);
                Assert.Contains($"<ram:DirectDebitMandateID>REF A-123</ram:DirectDebitMandateID>", content);
            }
        ...
        public void TestInNonDebitInvoiceTheDirectDebitFieldsShouldNotExist()
        {
            ...
            using (var stream = new MemoryStream())
            {
                ...
                Assert.DoesNotContain($"<ram:CreditorReferenceID>DE98ZZZ09999999999</ram:CreditorReferenceID>", content);
                Assert.DoesNotContain($"<ram:DirectDebitMandateID>REF A-123</ram:DirectDebitMandateID>", content);
            }
