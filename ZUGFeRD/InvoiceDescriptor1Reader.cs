    ...
    internal class InvoiceDescriptor1Reader : IInvoiceDescriptorReader
    {
        ...
        public override InvoiceDescriptor Load(Stream stream)
        {
            ...
            InvoiceDescriptor retval = new InvoiceDescriptor
            {
                ...
                Type = EnumExtensions.StringToEnum<InvoiceType>(XmlUtils.NodeAsString(doc.DocumentElement, "//rsm:HeaderExchangedDocument/ram:TypeCode", nsmgr)),
            ...
            foreach (XmlNode node in doc.SelectNodes("//rsm:HeaderExchangedDocument/ram:IncludedNote", nsmgr))
            {
                ...
                ContentCodes contentCode = EnumExtensions.StringToEnum<ContentCodes>(contentCodeAsString);
            ...
            if (XmlUtils.NodeExists(doc, "//ram:ApplicableSupplyChainTradeDelivery/ram:RelatedSupplyChainConsignment/ram:SpecifiedLogisticsTransportMovement/ram:ModeCode", nsmgr))
            {
                retval.TransportMode = EnumExtensions.StringToEnum<TransportModeCodes>(XmlUtils.NodeAsString(doc.DocumentElement, "//ram:ApplicableSupplyChainTradeDelivery/ram:RelatedSupplyChainConsignment/ram:SpecifiedLogisticsTransportMovement/ram:ModeCode", nsmgr));
            }
            ...
            retval.Currency = EnumExtensions.StringToEnum<CurrencyCodes>(XmlUtils.NodeAsString(doc.DocumentElement, "//ram:ApplicableSupplyChainTradeSettlement/ram:InvoiceCurrencyCode", nsmgr));

            // Multiple SpecifiedTradeSettlementPaymentMeans may exist for each account/institution // TODO
            // BG-16 0..unbounded PAYMENT INSTRUCTIONS according to EN16931
            foreach (XmlNode node in doc.DocumentElement.SelectNodes("//ram:ApplicableSupplyChainTradeSettlement/ram:SpecifiedTradeSettlementPaymentMeans", nsmgr))
            {
                ...
                var paymentMeans = new SpecifiedTradeSettlementPaymentMeans
                {
                    ...
                    SEPACreditorIdentifier = XmlUtils.NodeAsString(node, "./ram:ID", nsmgr),
                    SEPAMandateReference = XmlUtils.NodeAsString(node, "./ram:ID/@schemeAgencyID", nsmgr),
                };
                ...
                var debtorAccount = node.SelectSingleNode(
                    "./ram:PayerPartyDebtorFinancialAccount",
                    nsmgr);
                var debtorFinancialInstitution = node.SelectSingleNode(
                ...
                if (creditorAccount != null || creditorFinancialInstitution != null)
                {
                    ...
                    // BT-85 0..1 Payment account name
                    var accountName = XmlUtils.NodeAsString(debtorAccount, "./ram:AccountName", nsmgr);

                    // BT-84-0 0..1 National account number (not SEPA)
                    var proprietaryId = XmlUtils.NodeAsString(debtorAccount, "./ram:ProprietaryID", nsmgr);

                    // BT-86 1..1 Payment service provider identifier
                    var bic = XmlUtils.NodeAsString(debtorFinancialInstitution, "./ram:BICID", nsmgr);

                    // Legacy payment service provider name
                    var bankName = XmlUtils.NodeAsString(debtorFinancialInstitution, "./ram:Name", nsmgr);

                    // Legacy bank routing ID for German banking systems
                    var bankleitzahl = XmlUtils.NodeAsString(debtorFinancialInstitution, "./ram:GermanBankleitzahlID", nsmgr);
            ...
            foreach (XmlNode node in doc.SelectNodes("//ram:SpecifiedTradeAllowanceCharge", nsmgr))
            {
                ...
                {
                    retval._AddTradeCharge(XmlUtils.NodeAsDecimal(node, ".//ram:BasisAmount", nsmgr, 0).Value,
                                           retval.Currency,
                                           ...
                                           EnumExtensions.StringToNullableEnum<ChargeReasonCodes>(XmlUtils.NodeAsString(node, "./ram:ReasonCode", nsmgr)));
                }
                else // allowance
                {
                    retval._AddTradeAllowance(XmlUtils.NodeAsDecimal(node, ".//ram:BasisAmount", nsmgr, 0).Value,
                                              retval.Currency,
                                              XmlUtils.NodeAsDecimal(node, ".//ram:ActualAmount", nsmgr, 0).Value,
                                              XmlUtils.NodeAsString(node, ".//ram:Reason", nsmgr),
                                              EnumExtensions.StringToNullableEnum<TaxTypes>(XmlUtils.NodeAsString(node, ".//ram:CategoryTradeTax/ram:TypeCode", nsmgr)),
                                              EnumExtensions.StringToNullableEnum<TaxCategoryCodes>(XmlUtils.NodeAsString(node, ".//ram:CategoryTradeTax/ram:CategoryCode", nsmgr)),
                                              XmlUtils.NodeAsDecimal(node, ".//ram:CategoryTradeTax/ram:ApplicablePercent", nsmgr, 0).Value,
                                              EnumExtensions.StringToNullableEnum<AllowanceReasonCodes>(XmlUtils.NodeAsString(node, "./ram:ReasonCode", nsmgr)));
                }
        ...
        private static TradeLineItem _parseTradeLineItem(XmlNode tradeLineItem, XmlNamespaceManager nsmgr = null)
        {
            ...
            foreach (XmlNode referenceNode in referenceNodes)
            {
                ...
                item.AddAdditionalReferencedDocument(
                    ...
                    typeCode: EnumExtensions.StringToEnum<AdditionalReferencedDocumentTypeCode>(typeCodeAsString),
                    code: EnumExtensions.StringToNullableEnum<ReferenceTypeCodes>(codeAsString),
        ...
        private static Party _nodeAsParty(XmlNode baseNode, string xpath, XmlNamespaceManager nsmgr = null)
        {
            ...
            {
                ID = new GlobalID(null, XmlUtils.NodeAsString(node, "ram:ID", nsmgr)),
